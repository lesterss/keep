...
Но если нужно включить файл в репозиторий, и при этом игнорировать изменения — используйте
...
```
### Откатится на другой комит 
```
git reset --hard комит до которого надо откатить
git push -f  # И все
```
Или более лучший вариант 
```
git reflog смотрим список последних комитов находим нужный и 
git reset --hard HEAD@{1}  --- {1} . это номер нужного комита из списка выше
git push --force
```
...
```
Или Можно использовать git reset, вот так:
``` bash
git reset --hard HEAD~1 # Стоит заметить, что это “ядерный” способ, который отменит все изменения.
